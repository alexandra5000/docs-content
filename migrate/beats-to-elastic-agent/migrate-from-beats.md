...
For more information about {{agent}} limitations, see [{{beats}} and {{agent}} capabilities](/reference/fleet/beats-agent-comparison.md).
...
3. Decide if it’s the right time to migrate to {{agent}}. Review the information under [{{beats}} and {{agent}} capabilities](/reference/fleet/beats-agent-comparison.md). Make sure the integrations you need are supported and Generally Available, and that the output and features you require are also supported.
